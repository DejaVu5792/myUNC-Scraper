...
function __fish_myunc-scraper_complete
    ...
    set -lx COMP_TYPE
    if set -q _ARC_DEBUG
        myunc-scraper 8>&1 9>&2 1>&9 2>&1
    else
        myunc-scraper 8>&1 9>&2 1>/dev/null 2>&1
    end
end
complete --command myunc-scraper -f -a '(__fish_myunc-scraper_complete)'
