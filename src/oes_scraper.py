...
def get_available_subjects(page: Page) -> list[dict]:
    # Request tab
...
def scrape_oes_available_subjects() -> list[dict]:
    with sync_playwright() as p:
        browser, context = launch_browser(p)
        page = create_page(context)
        try:
            login_oes(page)
            return get_available_subjects(page)
        finally:
            browser.close()
...
def export_available_subjects_to_csv(subjects: list[dict], output_path: str = "available_subjects.csv") -> str:
    ...
    keys = ["code", "course_no", "unit", "title", "schedule", "type", "room", "teacher", "tally"]
